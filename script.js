...
function saveToCloud(){
  ...
  window._db.collection('users').doc(_user.uid).set({accounts: _accounts})
    .then(function(){ console.log('✅ سەیڤ کرا'); })
...
function checkUpdate(){
  ...
  setTimeout(function(){
    ...
    var bdy = document.getElementById('update-body');
    if (!el) return;
    if (ver) ver.textContent = 'وەرژن ' + SITE_VERSION;
    if (bdy) bdy.innerHTML  = UPDATE_NOTES.map(function(n){ return '<div class="update-note">• '+n+'</div>'; }).join('');
...
function renderList(){
  ...
  if (!list.length){
    el.innerHTML = '<div class="empty-msg" style="padding:28px 0">هیچ ئەکاونتێک نییە.<br>دووگمەی + بکەرەوە.</div>';
    return;
  }
